...
var HomePage = /** @class */ (function () {
    function HomePage(socket, router, http, toastController, ds) {
        this.socket = socket;
        this.router = router;
        this.http = http;
    ...
    HomePage.prototype.register = function () {
        ...
        this.http.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_5__["environment"].SERVER_URL + "/user", { user: { username: this.username, password: this.password } }).subscribe(function (data) {
            if (data.status !== 'danger') {
                _this.ds.setIsLogged(true);
                _this.ds.setUsername(_this.username);
                localStorage.setItem('username', _this.username);
                _this.user = _this.username;
                _this.username = "";
                _this.password = "";
                _this.presentToastWithOptions(data);
            }
            else
                _this.presentToastWithOptions({ message: "Username already taken.", status: "danger" });
        });
    };
    HomePage.prototype.login = function () {
        ...
        this.http.get(src_environments_environment__WEBPACK_IMPORTED_MODULE_5__["environment"].SERVER_URL + "/user/" + this.username)
            ...
            _this.presentToastWithOptions({ message: "Username doesn't exist.", status: "danger" });
        });
    };
    HomePage.prototype.createRoom = function () {
        var _this = this;
        var roomId = this.makeid(5);
        this.http.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_5__["environment"].SERVER_URL + "/playlist", { item: { title: roomId, description: 'test description', user: this.ds.getUsername() } })
            ...
            if (data) {
                _this.presentToastWithOptions(data);
                _this.router.navigate(['/tv', { id: roomId }]);
            }
        });
    };
    HomePage.prototype.joinRoom = function () {
        ...
        if (this.joinRoomId) {
            this.http.get(src_environments_environment__WEBPACK_IMPORTED_MODULE_5__["environment"].SERVER_URL + "/playlists")
                .subscribe(function (data) {
                console.log(data);
                if (data.filter(function (item) { return item.title === _this.joinRoomId; }).length)
                    _this.router.navigate(['/user', { id: _this.joinRoomId }]);
            });
        }
    };
    HomePage.prototype.makeid = function (length) {
        var result = '';
    ...
    HomePage.prototype.presentToastWithOptions = function (data) {
